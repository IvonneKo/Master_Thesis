...
class VimltsLinear(tf.keras.layers.Layer):
    ...
    def get_b_dist(self, num=1000):
        ...
        return tf.math.exp(log_q_w).numpy().squeeze(), b.numpy().squeeze()
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.use_bias:
            out = self.activation_(inputs @ w + b[:, None, :])     ##-in - b
        else:
...
class VimltsLinearFroz(tf.keras.layers.Layer):
    def __init__(self,
    ...
    def build(self, input_shape):
        ...
        self.k_beta_z = self.add_weight(name='k_beta_z',
                                        ...
                                        trainable=False)
        theta_prime = tf.stack([i(shape=shape) for i in self.k_thetas_], axis=-1)
        self.k_theta_prime = tf.Variable(initial_value=theta_prime, trainable=False)
        ...
        if self.use_bias:
            ...
            self.b_beta_z = self.add_weight(name='b_beta_z',
                                            ...
                                            trainable=False)
            b_theta_prime = tf.stack([i(shape=shape) for i in self.b_thetas_], axis=-1)
            self.b_theta_prime = tf.Variable(initial_value=b_theta_prime, trainable=False)
        super().build(input_shape)
